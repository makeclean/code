...
!------------------------------------------------------------------------------
        ! write the node array
        CALL WRITE_MERA_ARRAY(g_coord,SIZE(g_coord),id1,id2)
        ...
        !  WRITE(11,'(A)') "*ELEMENTS"
        CALL WRITE_MERA_ARRAY(g_num,SIZE(g_num),id1,id2)  
        !   IF(nod==20) THEN
        !    DO iel = 1, nels
...
!------------------------------------------------------------------------------
          ...
          CALL WRITE_MERA_ARRAY(rest,SIZE(REST),id1,id2)
...
!------------------------------------------------------------------------------
          CALL WRITE_MERA_ARRAY(no,SIZE(no),id1,id2)
          CALL WRITE_MERA_ARRAY(val,SIZE(val),id1,id2)
          ! need to write the loading file seperately due to the need to access 
          ! speicifc loadings on certain elements
